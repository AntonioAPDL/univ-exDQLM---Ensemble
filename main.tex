...
\end{itemize}
Throughout, $\phi(\cdot)$ and $\Phi(\cdot)$ denote the standard normal pdf and cdf; in source form we write them as $\phiN{\cdot}$ and $\PhiN{\cdot}$.
...
where
\begin{align}
g(\gamma)&=2\,\PhiN{-\lvert\gamma\rvert}\exp(\gamma^2/2),\label{eq:g_gamma}\\
...
\label{eq:vb_v_psi}
\end{align}
Derivation note: by the CAVI rule \eqref{eq:cavi_rule}, $q(v_t)$ is derived from
$\E_{-v_t}[\log p(y_t\mid\cdot)+\log p(v_t\mid\sigma)]$, so the coefficients
$\chi_{v,t}$ and $\psi_{v,t}$ cannot depend on $\E[1/v_t]$ itself. Including
$m_{1/v,t}$ would make the update circular.
...
\tilde f(\sigma,\gamma)
=
&-(a_\sigma+1+\tfrac{3T}{2})\log\sigma - \frac{b_\sigma}{\sigma} - \frac{D_3}{\sigma}
-\frac{T}{2}\log B(p)\nonumber\\
...
\label{eq:f_tilde}
\end{align}
The term $-D_3/\sigma$ is the contribution of $-\sum_{t=1}^T \E[v_t]/\sigma$ from
$\E[\log p(v_t\mid\sigma)]$ with $D_3=\sum_{t=1}^T\E[v_t]$.
